                ...
                (let ((user-notifications (gethash user *notifications*)))
                  (prog1 (json-response 200 `(:notifications ,(mapcar (lambda (n)
                                                                        (let ((ht (make-hash-table :test 'equal)))
                                                                          (setf (gethash "from" ht) (notification-from n))
                                                                          (setf (gethash "content" ht) (notification-content n))
                                                                          (setf (gethash "time" ht) (notification-time n))
                                                                          (setf (gethash "channel" ht) (notification-channel n))
                                                                          ht))
